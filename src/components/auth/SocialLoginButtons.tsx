...
export function SocialLoginButtons({ onSocialLogin }: SocialLoginButtonsProps) {
  ...
  useEffect(() => {
    const loadActiveProviders = async () => {
      ...
      if (data) {
        setActiveProviders(data as ActiveProvider[]);
      }
    };
  ...
  // Don't show anything if no providers are active
  if (activeProviders.length === 0) {
    return null;
  }

  const isProviderActive = (provider: string) => 
    activeProviders.some(p => p.provider === provider && p.is_enabled);
      ...
      <div className="mt-6 grid grid-cols-3 gap-3">
        ...
        {isProviderActive('facebook') && (
          <Button
            ...
            className="w-full !bg-[#1877F2] hover:!bg-[#166FE5] !text-white border border-[#1466D8] shadow-sm"
          >
        ...
        {isProviderActive('apple') && (
          <Button
            type="button"
            onClick={() => onSocialLogin("apple")}
            className="w-full !bg-black hover:!bg-gray-900 !text-white border border-gray-700 shadow-sm"
          >
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
